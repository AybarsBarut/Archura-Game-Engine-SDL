...
namespace Archura {
    ...
    void FPSConsoleCommands::RegisterGameplayCommands() {
        ...
        console.RegisterCommand(std::make_shared<ConsoleCommand>(
            ...
            [](const std::vector<std::string>& args) {
                if (args.size() < 4) {
                    std::cout << "[Game] Usage: teleport [entity_name] [x] [y] [z]\n";
                    return;
                }
                ...
                try {
                    ...
                    std::cout << "[Game] Invalid coordinates.\n";
                    return;
                }

                auto* scene = Application::Get().GetScene();
                if (!scene) {
                    std::cout << "[Game] Error: No active scene.\n";
                    return;
                }
                ...
                if (targetEntity) {
                    ...
                    if (transform) {
                        ...
                        std::cout << "[Game] Teleported '" << entityName << "' to " << x << ", " << y << ", " << z << "\n";
                    } else {
                        std::cout << "[Game] Entity '" << entityName << "' has no Transform component.\n";
                    }
                } else {
                    std::cout << "[Game] Entity '" << entityName << "' not found.\n";
                }
            }
        ));
