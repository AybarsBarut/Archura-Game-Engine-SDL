...
Editor::Editor()
    ...
    , m_EditorCamera(glm::vec3(0.0f, 8.0f, 20.0f))
{}
...
void Editor::RefreshAssetCache() {
    ...
    scanDir("assets/textures", m_CachedTextureFiles, [](const std::string& e){
        return e == ".jpg" || e == ".png" || e == ".tga" || e == ".bmp" || e == ".jpeg";
    });
...
void Editor::BeginDockSpace() {
    ...
    // BeginDockSpace is intentionally a no-op; layout is managed via SetNextWindowPos.
}
...
void Editor::DrawEditorUI(Scene* scene) {
    ...
    if (m_AssetCacheDirty) RefreshAssetCache();
    ...
    if (m_ShowSceneHierarchy) {
        ImGui::SetNextWindowPos (ImVec2(vp->WorkPos.x,        Py),               m_HierarchyLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(Lw, H - Bh - Tb),                        m_HierarchyLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        DrawSceneHierarchy(scene);
    }
    if (m_ShowInspector) {
        ImGui::SetNextWindowPos (ImVec2(vp->WorkPos.x + W - Rw, Py),             m_InspectorLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(Rw, H - Tb),                             m_InspectorLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        DrawInspector(scene);
    }
    if (m_ShowProjectPanel || m_ShowConsole) {
        ImGui::SetNextWindowPos (ImVec2(vp->WorkPos.x + Lw,  vp->WorkPos.y + H - Bh), m_ConsoleLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(W - Lw - Rw, Bh),                        m_ConsoleLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        if (m_ShowConsole) DrawConsolePanel();
    }
    if (m_ShowProjectPanel) {
        ImGui::SetNextWindowPos (ImVec2(vp->WorkPos.x, vp->WorkPos.y + H - Bh), m_ProjectPanelLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        ImGui::SetNextWindowSize(ImVec2(Lw, Bh),                                 m_ProjectPanelLocked ? ImGuiCond_Always : ImGuiCond_FirstUseEver);
        DrawProjectPanel();
    }
...
void Editor::DrawToolbar(Scene* /*scene*/) {
    ...
    if (!playing) {
        ...
        ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.24f, 0.66f, 0.40f, 1.0f));
        ImGui::PushStyleColor(ImGuiCol_ButtonActive,  ImVec4(0.14f, 0.40f, 0.24f, 1.0f));
    ...
    // ── Right: FPS stat ────────────────────────────────────────────────────
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_Text, fps >= 60.0f
            ? ImVec4(0.37f, 0.85f, 0.52f, 1.0f)
        ...
        if (ImGui::BeginMenu("File")) {
            ...
            if (ImGui::MenuItem("Save Project", "Ctrl+S")) {
                if (scene) {
                    ...
                    ProjectSerializer::SaveProject("games/ArchuraGame/project.gameproj", cfg, scene);
                    Log("Project saved.");
                }
            }
...
void Editor::DrawOverlay(Scene* scene, Camera* gameCamera) {
    ...
    ImVec2 pos(io.DisplaySize.x - DIST, DIST + 20.0f);
    ...
    if (ImGui::Begin("##Overlay", nullptr, flags)) {
        ...
        ImU32     colors[3]= {IM_COL32(230,60,60,255), IM_COL32(60,220,60,255), IM_COL32(80,110,240,255)};
...
void Editor::DrawSceneHierarchy(Scene* scene) {
    ...
    if (m_HierarchyLocked) window_flags |= ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse;
    ImGui::Begin("Scene Hierarchy", nullptr, window_flags);

    if (!scene) { ImGui::TextDisabled("No active scene"); ImGui::End(); return; }

    const auto& entities = scene->GetEntities();
    ImGui::TextDisabled("%zu entities", entities.size());
    ImGui::SameLine();
    if (ImGui::SmallButton("+ Add")) ImGui::OpenPopup("AddEntityPopup");

    ImGui::SameLine(ImGui::GetWindowWidth() - 35.0f);
    if (ImGui::Button("...##HierMenu")) ImGui::OpenPopup("HierConfig");
    ...
    if (ImGui::BeginPopup("AddEntityPopup")) {
        ...
        if (m_CachedModelFiles.empty()) ImGui::TextDisabled("(none in assets/models)");
        ImGui::EndPopup();
    }
    ...
    auto entityIcon = [](Entity* e) -> const char* {
        if (e->GetComponent<LightComponent>()) return "  ";
        if (e->GetComponent<MeshRenderer>())   return "  ";
        return "  ";
    };
    ...
    std::function<void(Entity*)> DrawNode = [&](Entity* entity) {
        ...
        ImVec2 rowMax = ImVec2(rowMin.x + ImGui::GetContentRegionAvail().x, rowMin.y + ImGui::GetTextLineHeightWithSpacing());
        float rowAlpha = (int)(entity->GetID()) % 2 == 0 ? 0.04f : 0.0f;
        ImGui::GetWindowDrawList()->AddRectFilled(rowMin, rowMax, IM_COL32(255,255,255, (int)(rowAlpha * 255)));

        if (isLookedAt) ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(0.37f, 0.85f, 0.52f, 1.0f));

        char label[192];
        snprintf(label, sizeof(label), "%s%s##%u", entityIcon(entity), entity->GetName().c_str(), entity->GetID());
        bool opened = ImGui::TreeNodeEx((void*)(uintptr_t)entity->GetID(), flags, "%s", label);
        ...
        if (ImGui::BeginDragDropTarget()) {
            if (const ImGuiPayload* pl = ImGui::AcceptDragDropPayload("ENTITY_DRAG")) {
                ...
                if (de && de != entity) de->SetParent(entity);
            }
        ...
        if (ImGui::BeginPopupContextItem()) {
            if (ImGui::MenuItem("Unparent")) entity->SetParent(nullptr);
        ...
        if (opened) {
            for (auto* child : entity->GetChildren()) DrawNode(child);
            ImGui::TreePop();
        }
    };

    for (const auto& ePtr : entities)
        if (ePtr->GetParent() == nullptr) DrawNode(ePtr.get());
    ...
    if (ImGui::BeginDragDropTarget()) {
        if (const ImGuiPayload* pl = ImGui::AcceptDragDropPayload("ENTITY_DRAG")) {
            EntityID did = *(const EntityID*)pl->Data;
            if (Entity* de = scene->GetEntity(did)) de->SetParent(nullptr);
        }
        ImGui::EndDragDropTarget();
    }
...
void Editor::DrawInspector(Scene* scene) {
    ImGuiWindowFlags window_flags = 0;
    if (m_InspectorLocked) window_flags |= ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse;
    ImGui::Begin("Inspector", nullptr, window_flags);

    auto DrawMenu = [&]() {
        ImGui::SameLine(ImGui::GetWindowWidth() - 35.0f);
        if (ImGui::Button("...##InspMenu")) ImGui::OpenPopup("InspConfig");
    ...
    if (ImGui::InputText("##Name", nameBuf, sizeof(nameBuf),
        ...
        if (strlen(nameBuf) > 0) m_SelectedEntity->SetName(nameBuf);
    ...
    if (tf && ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
        // Helper: drag + inline reset button
        auto DragVec3WithReset = [](const char* label, glm::vec3& v, float defaultVal, float speed) {
            ...
            char rbId[64]; snprintf(rbId, sizeof(rbId), "R##%s", label);
            if (ImGui::SmallButton(rbId)) v = glm::vec3(defaultVal);
    ...
    if (mr) {
        if (ImGui::SmallButton("Modify Mesh Geometry")) m_ObjectTool.SetOpen(true);
        ImGui::Spacing();
    }
    ...
    if (mr && ImGui::CollapsingHeader("Texture")) {
        if (ImGui::SmallButton("Refresh##Tex")) m_AssetCacheDirty = true;
        ImGui::SameLine();
        ...
        if (ImGui::BeginListBox("##Textures", ImVec2(-1, 120.0f))) {
            for (int i = 0; i < (int)m_CachedTextureFiles.size(); ++i) {
                ...
                if (ImGui::Selectable(m_CachedTextureFiles[i].c_str(), isSel)) {
                    ...
                    if (tex) { mr->texture = tex; Log("Texture: " + p); }
                }
        ...
        if (mr->texture) {
            ...
            if (ImGui::SmallButton("Remove")) mr->texture = nullptr;
            ImGui::EndGroup();
        }
    }
    ...
    if (weapon && ImGui::CollapsingHeader("Weapon")) {
        const char* wTypes[] = {"Rifle","Pistol","Knife","Grenade","Shotgun","Sniper"};
        int wt = (int)weapon->type;
        if (ImGui::Combo("Type", &wt, wTypes, 6)) weapon->SwitchWeapon((Weapon::WeaponType)wt);
        ...
        ImGui::Separator();
        ImGui::Text("Ammo: %d/%d  Total: %d",
            weapon->stats.currentMag, weapon->stats.magSize, weapon->stats.totalAmmo);
    }

    // ── Projectile ─────────────────────────────────────────────────────────
    auto* proj = m_SelectedEntity->GetComponent<Projectile>();
    if (proj && ImGui::CollapsingHeader("Projectile")) {
        ImGui::Text("Speed %.2f  Damage %.2f  Life %.2f",
            proj->speed, proj->damage, proj->lifetime);
    ...
    if (lc && ImGui::CollapsingHeader("Light")) {
        ...
        int lt = (int)lc->type;
        if (ImGui::Combo("Type", &lt, ltypes, 3)) lc->type = (LightComponent::Type)lt;
    ...
    if (sky && ImGui::CollapsingHeader("Skybox")) {
        const char* faceLabels[] = {"Right","Left","Top","Bottom","Front","Back"};
        for (int i = 0; i < 6; ++i) {
            char buf[256];
        ...
        if (ImGui::Button("Reload Skybox")) sky->shouldReload = true;
    }
    ...
    if (ImGui::CollapsingHeader("Script", ImGuiTreeNodeFlags_DefaultOpen)) {
        if (sc) {
            ...
            ImGui::PushStyleColor(ImGuiCol_FrameBg, ImVec4(0.12f, 0.22f, 0.12f, 1.0f));
            if (ImGui::InputText("Class##SC", scBuf, sizeof(scBuf),
                                 ImGuiInputTextFlags_EnterReturnsTrue))
                sc->className = scBuf;
            if (ImGui::IsItemDeactivatedAfterEdit()) sc->className = scBuf;
            ImGui::PopStyleColor();
            ImGui::SameLine();
            ImGui::PushStyleColor(ImGuiCol_Button,        ImVec4(0.55f, 0.15f, 0.15f, 1.0f));
            ...
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(0.22f, 0.60f, 0.22f, 1.0f));
...
void Editor::DrawProjectPanel() {
    ImGuiWindowFlags window_flags = 0;
    if (m_ProjectPanelLocked) window_flags |= ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse;
    ImGui::Begin("Project", nullptr, window_flags);

    if (m_CurrentProjectDir != m_BaseProjectDir) {
        if (ImGui::SmallButton("..")) m_CurrentProjectDir = m_CurrentProjectDir.parent_path();
        ImGui::SameLine();
    }
    if (ImGui::SmallButton("Refresh")) m_AssetCacheDirty = true;
    ImGui::SameLine();
    ImGui::TextDisabled("%s", m_CurrentProjectDir.filename().string().c_str());

    ImGui::SameLine(ImGui::GetWindowWidth() - 35.0f);
    if (ImGui::Button("...##ProjMenu")) ImGui::OpenPopup("ProjConfig");
    ...
    for (auto& entry : std::filesystem::directory_iterator(m_CurrentProjectDir, ec)) {
        ...
        if (entry.is_directory(ec)) {
            if (ImGui::Selectable((name + "/").c_str(), false, ImGuiSelectableFlags_AllowDoubleClick))
                if (ImGui::IsMouseDoubleClicked(0)) m_CurrentProjectDir /= p.filename();
        } else {
...
void Editor::DrawConsolePanel() {
    ImGuiWindowFlags window_flags = 0;
    if (m_ConsoleLocked) window_flags |= ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse;
    ...
    ImGui::SameLine(ImGui::GetWindowWidth() - 35.0f);
    if (ImGui::Button("...##ConsMenu")) ImGui::OpenPopup("ConsConfig");
    ...
    ImGui::BeginChild("##ConsoleLogs", ImVec2(0, -footerH), false, ImGuiWindowFlags_HorizontalScrollbar);

    for (const auto& log : m_ConsoleLogs) {
        // Colour-code by prefix
        if (log.rfind("[WARN]", 0) == 0)
...
void Editor::DrawPerformanceMetrics(float deltaTime, float fps) {
    ...
    ImGui::Separator();
    ImGui::PlotLines("##FT", frameTimes, 90, ftIdx, "ms", 0.0f, 33.0f, ImVec2(-1, 60));
...
void Editor::SpawnEntity(Scene* scene, const std::string& type, const std::string& path) {
    ...
    if      (type == "Cube")    mr->mesh = Mesh::CreateCube();
    else if (type == "Sphere")  { mr->mesh = Mesh::CreateSphere(0.5f); }
    else if (type == "Capsule") { mr->mesh = Mesh::CreateCapsule(0.5f, 2.0f); col->size = {1,2,1}; }
    else if (type == "Stairs")  { mr->mesh = Mesh::CreateStairs(1,1,2,5);     col->size = {1,1,2}; }
    else if (type == "Ramp")    { mr->mesh = Mesh::CreateRamp(1,1,2);          col->size = {1,1,2}; }
    else if (type == "Light") {
        ...
        if      (ext == ".obj") mr->mesh = Mesh::LoadFromOBJ(path);
        else if (ext == ".fbx") mr->mesh = Mesh::LoadFromFBX(path);
        else {
...
void Editor::DrawModelDirectory(Scene* scene, const std::filesystem::path& dir) {
    std::error_code ec;
...
Entity* Editor::PickEntityAtScreenPos(Scene* scene, Input* input,
                                      int screenW, int screenH)
{
    if (!scene || !input)           return nullptr;
    if (screenW <= 0 || screenH <= 0) return nullptr;

    // Only fire on the frame the LMB is pressed (not while held)
    if (!input->IsMouseButtonPressed(SDL_BUTTON_LEFT)) return nullptr;

    // If any ImGui window wants the mouse (e.g. panel clicked), skip
    if (ImGui::GetIO().WantCaptureMouse) return nullptr;
    ...
    for (const auto& ePtr : scene->GetEntities()) {
        ...
        halfSize *= tf->scale;
        glm::vec3 worldCenter = tf->position + (bc ? bc->center * tf->scale : glm::vec3(0.0f));
...
void Editor::DrawTransformGizmo(int screenW, int screenH) {
    ...
    for (int i = 0; i < 3; ++i) m_GizmoRotArcSSValid[i] = false;
    ...
    glm::mat4 vp = m_EditorCamera.GetProjectionMatrix(asp) * m_EditorCamera.GetViewMatrix();
    ...
    auto proj2ss = [&](const glm::vec3& wp, glm::vec2& out) -> bool {
        ...
        if (std::abs(ndc.x) > 1.5f || std::abs(ndc.y) > 1.5f) return false;
        out = { (ndc.x + 1.0f) * 0.5f * screenW,
    ...
    ImU32 tNorm[3] = { IM_COL32(230,60,60,255), IM_COL32(60,220,60,255), IM_COL32(80,110,240,255) };
    ImU32 rNorm[3] = { IM_COL32(200,60,60,160), IM_COL32(60,200,60,160), IM_COL32(80,90,220,160)  };
    ...
    for (int i = 0; i < 3; ++i) {
        glm::vec2 tss;
        if (!proj2ss(tipW[i], tss)) continue;
        m_GizmoTransTipSS[i] = tss;
        glm::vec2 dir = tss - oss;
        float len = glm::length(dir);
        if (len > 0.001f) m_GizmoAxisDir2D[i] = dir / len;
        ...
        dl->AddText(ImVec2(tss.x + 10.f*sx - 3.f, tss.y + 10.f*sy - 6.f), col, lbl[i]);
    }
    ...
    for (int ax = 0; ax < 3; ++ax) {
        ...
        bool anyOk = false;
        glm::vec2 prev; bool prevOk = false;
        for (int s = 0; s <= ROT_ARC_SEGS; ++s) {
            float t = (float)s / (float)ROT_ARC_SEGS * TWO_PI;
            float ct = std::cos(t), st = std::sin(t);
            glm::vec3 wp;
            if      (ax == 0) wp = origin + glm::vec3(0,       rotR*ct, rotR*st);
            else if (ax == 1) wp = origin + glm::vec3(rotR*ct, 0,       rotR*st);
            else              wp = origin + glm::vec3(rotR*ct, rotR*st, 0      );
            glm::vec2 ss;
            ...
            if (ok && prevOk) dl->AddLine(toImVec(prev), toImVec(ss), col, lw);
            prev = ss; prevOk = ok;
        }
...
void Editor::ProcessGizmoInput(Input* input, int screenW, int screenH) {
    ...
    if (!m_SelectedEntity || !input || !m_GizmoSSValid) {
        m_GizmoActive = GizmoOp::None; return;
    }
    auto* tf = m_SelectedEntity->GetComponent<Transform>();
    if (!tf) { m_GizmoActive = GizmoOp::None; return; }

    glm::vec2 mp  = input->GetMousePosition();
    bool lmbDown    = input->IsMouseButtonDown(SDL_BUTTON_LEFT);
    bool lmbPressed = input->IsMouseButtonPressed(SDL_BUTTON_LEFT);

    auto distSeg = [](const glm::vec2& p, const glm::vec2& a, const glm::vec2& b) -> float {
    ...
    if (m_GizmoActive == GizmoOp::None) {
        for (int i = 0; i < 3; ++i) {
            if (distSeg(mp, m_GizmoOriginSS, m_GizmoTransTipSS[i]) < 10.0f) {
                m_GizmoHovered = (GizmoOp)((int)GizmoOp::TransX + i); break;
            }
        }
        if (m_GizmoHovered == GizmoOp::None) {
            for (int axis = 0; axis < 3; ++axis) {
                if (!m_GizmoRotArcSSValid[axis]) continue;
                float minD = 1e9f;
                for (int s = 0; s < ROT_ARC_SEGS; ++s)
                    minD = std::min(minD, distSeg(mp, m_GizmoRotArcSS[axis][s],
                                                      m_GizmoRotArcSS[axis][(s+1)%ROT_ARC_SEGS]));
                if (minD < 9.0f) { m_GizmoHovered = (GizmoOp)((int)GizmoOp::RotX + axis); break; }
            }
        }
    }

    // Begin drag
    if (lmbPressed && !ImGui::GetIO().WantCaptureMouse && m_GizmoHovered != GizmoOp::None) {
        ...
        m_GizmoDragScale = (2.0f * d * std::tan(glm::radians(m_EditorCamera.GetFOV()) * 0.5f)) / (float)screenH;
    }
    ...
    if (m_GizmoActive != GizmoOp::None && lmbDown) {
        ...
        if (opIdx >= (int)GizmoOp::TransX && opIdx <= (int)GizmoOp::TransZ) {
            ...
            float proj2d = glm::dot(mp - m_GizmoDragMouseStart, m_GizmoAxisDir2D[axis]);
            ...
            m_GizmoDragMousePrev = mp;
            float angleDeg = 0.0f;
            if      (axis == 0) angleDeg = -delta.y * 0.35f;  // X: vertical drag
            else if (axis == 1) angleDeg =  delta.x * 0.35f;  // Y: horizontal drag
            else                angleDeg = -delta.x * 0.35f;  // Z: horizontal drag
            tf->rotation[axis] += angleDeg;
        }
    }

    if (!lmbDown) m_GizmoActive = GizmoOp::None;
}
...
void Editor::PushUndoSnapshot(Entity* entity) {
    ...
    if (!tf) return;
    if ((int)m_UndoStack.size() >= UNDO_LIMIT) m_UndoStack.pop_front();
    UndoSnapshot snap;
    snap.entity = entity;
...
void Editor::UndoLastAction(Scene* /*scene*/) {
    if (m_UndoStack.empty()) { Log("[Undo] Yok."); return; }
...
void Editor::CopySelected() {
    ...
    if (tf) { m_Clipboard.pos = tf->position; m_Clipboard.rot = tf->rotation; m_Clipboard.scale = tf->scale; }
    if (mr) { m_Clipboard.mesh = mr->mesh; m_Clipboard.texture = mr->texture; m_Clipboard.color = mr->color; }
    if (bc) { m_Clipboard.hasBc = true; m_Clipboard.bcSize = bc->size; m_Clipboard.bcCenter = bc->center; m_Clipboard.bcTrigger = bc->isTrigger; }
    if (sc) { m_Clipboard.hasScript = true; m_Clipboard.scriptClass = sc->className; }
    Log("[Kopyala] " + m_SelectedEntity->GetName());
}

void Editor::PasteClipboard(Scene* scene) {
    if (!scene || !m_Clipboard.valid) return;
    Entity* e = scene->CreateEntity(m_Clipboard.name);
    auto*   tf = e->GetComponent<Transform>();
    if (tf) {
        tf->position = m_Clipboard.pos + glm::vec3(1.0f, 0.0f, 1.0f); // slight offset
...
void Editor::ProcessEditorShortcuts(Input* input, Scene* scene) {
    if (!input) return;
    if (ImGui::GetIO().WantTextInput) return; // typing in an ImGui field – don't steal keys
    ...
    if (!ctrl && input->IsKeyJustPressed(SDL_SCANCODE_DELETE) && m_SelectedEntity && scene) {
        scene->DestroyEntity(m_SelectedEntity->GetID());
        m_SelectedEntity = nullptr;
        Log("[Sil] Entity silindi.");
    }
    ...
    if (!ctrl && input->IsKeyJustPressed(SDL_SCANCODE_F) && m_SelectedEntity) {
        ...
        if (tf) {
            ...
            glm::vec3 dir = glm::normalize(m_EditorCamera.GetPosition() - tf->position);
